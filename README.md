...
A Houdini-style procedural node graph for building AI agent pipelines and 3D scenes — scriptable in Python, fast in Rust, visual in the browser.

---

## What it is

Blacknode lets you wire together nodes the same way Houdini wires together geometry operations. Each node does one thing — call an LLM, load a mesh, filter a list, fetch a URL — and the graph cooks lazily: only nodes whose inputs changed are re-evaluated.
...
| Concept | Description |
|---|---|
| **Lazy pull evaluation** | Cooking a port pulls from upstream; unchanged nodes return cached results |
| **Dirty-flag propagation** | Changing a param or connection marks only the affected downstream nodes stale |
...
│   ├── blacknode-core/      # DAG, Node trait, lazy cook, dirty flags
...
│       └── scene.py  # SceneRoot, MeshLoad, Transform, MergeScene
├── examples/
...
│   └── scene_build.py        # scene assembly graph
...
- [x] Built-in node library (ai, flow, io, scene, core)
