...
def export_workflow_python(data: Mapping[str, Any], *, style: str = "flat") -> str:
    ...
    entry_name = names.get(node_id, node_id)
    runtime_root = Path(__file__).resolve().parents[2]
    lines = [
        "from __future__ import annotations",
        "",
        f"# Generated by Blacknode from workflow: {workflow_name}",
        f"# Entrypoint: {entry_name}.{port}",
        ...
        "from pathlib import Path",
        "",
        "# Quiet OpenCV's camera-probe chatter (the 'index out of range' lines a",
        "# multi-camera machine prints while discovering devices) before cv2 loads.",
        ...
        "",
        "",
        "def _bootstrap_blacknode_runtime() -> None:",
        f"    embedded_root = Path({str(runtime_root)!r})",
        "    roots: list[Path] = []",
        "    blacknode_home = os.environ.get('BLACKNODE_HOME')",
        "    if blacknode_home:",
        "        roots.append(Path(blacknode_home).expanduser())",
        ...
        "        if (root / 'python' / 'blacknode').exists():",
        "            _prepend_sys_path(root / 'python')",
        "        if (root / 'blacknode').exists():",
        "            _prepend_sys_path(root)",
        "",
        "",
        "_bootstrap_blacknode_runtime()",
        "",
        "try:",
        ...
        "            'Could not import the Blacknode runtime. Set BLACKNODE_HOME=/path/to/Blacknode, '",
        "            'run this script from the Blacknode repo, or install Blacknode into this Python environment.'",
        "        ) from exc",
        "    raise",
        "",
        "",
        ...
        "",
        "",
        "def _workflow_uses_live_runtime() -> bool:",
        "    node_meta = _WORKFLOW.get('node_meta') or {}",
        "    for meta in node_meta.values():",
        "        if not isinstance(meta, dict):",
        "            continue",
        "        if meta.get('type') not in _BLACKNODE_LIVE_RUNTIME_NODE_TYPES:",
        "            continue",
        "        params = meta.get('params') or {}",
        "        if meta.get('type') == 'MapEnvironment' and str(params.get('action') or 'status').strip().lower() != 'start':",
        "            continue",
        "        if str(params.get('action') or 'start').strip().lower() != 'stop':",
        "            return True",
        "    return False",
        "",
        "",
        "def _stop_blacknode_runtime_services() -> None:",
        "    global _blacknode_live_runtime_stopped",
        "    if _blacknode_live_runtime_stopped:",
        "        return",
        ...
        "        'blacknode.pkg.blacknode_perception.slam.adapters.ros2.mapping',",
        ...
        "            stop = getattr(module, 'stop_runtime_services', None)",
        "            if stop is not None:",
        "                stop()",
        "        except ModuleNotFoundError:",
        "            pass",
        "        except Exception as exc:",
        "            print(f'Blacknode runtime stop warning ({module_name}): {exc}', file=sys.stderr)",
        "",
        "",
        "def _print_live_status() -> None:",
        "    # After cooking, say plainly what is actually streaming and what is",
        "    # not - the entrypoint value alone (often a URL) does not reveal a",
        "    # camera that opened but never delivered a frame.",
        "    import importlib",
        "    time.sleep(3)  # let a camera warm up or fail before reporting",
        "    rows = []",
        "    for module_name, status_name in (",
        "        ('blacknode.pkg.blacknode_perception.cv2_runtime', 'runtime_status'),",
        "        ('blacknode.pkg.blacknode_ros2.ros2_runtime', 'runtime_status'),",
        "        ('blacknode.pkg.blacknode_perception.slam.adapters.ros2.mapping', 'runtime_status'),",
        "        ('blacknode.pkg.blacknode_robot.robot', 'runtime_status'),",
        "        ('blacknode.pkg.blacknode_skills.follow.leader_follower_runtime', 'leader_follower_runtime_status'),",
        "    ):",
        "        try:",
        "            status = getattr(importlib.import_module(module_name), status_name)()",
        "        except Exception:",
        "            continue",
        "        groups = [status] if isinstance(status, list) else [",
        "            status.get(key, []) or []",
        "            for key in ('cv2_streams', 'streams', 'reasoning_streams', 'managed_runs')",
        "        ] if isinstance(status, dict) else []",
        "        for group in groups:",
        "            for item in group:",
        "                if not isinstance(item, dict):",
        "                    continue",
        "                name = item.get('stream_id') or item.get('run_id') or item.get('topic') or item.get('device') or '?'",
        "                live = item.get('streaming') is not False and item.get('ok') is not False",
        "                report = str(item.get('report') or '').strip()",
        "                mark = 'LIVE ' if live else 'NOT LIVE '",
        "                rows.append(f'  [{mark}] {name}: {report}' if report else f'  [{mark}] {name}')",
        "    if rows:",
        "        print('Blacknode stream status:', file=sys.stderr)",
        "        for row in rows:",
        "            print(row, file=sys.stderr)",
        "    else:",
        "        print('Blacknode: no active streams - nothing went live. Check the node reports above.', file=sys.stderr)",
        "",
        "",
        "def _raise_blacknode_stop(_signum, _frame) -> None:",
        "    raise KeyboardInterrupt",
        "",
        "",
        "atexit.register(_stop_blacknode_runtime_services)",
        "",
        "",
        "def _hold_live_runtime_if_needed() -> None:",
        "    if not _workflow_uses_live_runtime():",
        "        return",
        "    if hasattr(signal, 'SIGTERM'):",
        "        signal.signal(signal.SIGTERM, _raise_blacknode_stop)",
        "    _print_live_status()",
        "    print('Blacknode live runtime is active. Press Ctrl-C to stop streams.', file=sys.stderr)",
        "    try:",
        "        while True:",
        "            time.sleep(1)",
        "    except KeyboardInterrupt:",
        "        print('Stopping Blacknode live runtime...', file=sys.stderr)",
        "    finally:",
        "        _stop_blacknode_runtime_services()",
        "",
    ]
...
def _export_workflow_python_flat(
    ...
    lines.extend([
        ...
        "    print(result)",
        "    _hold_live_runtime_if_needed()",
        "finally:",
        "    if _workflow_uses_live_runtime():",
        "        _stop_blacknode_runtime_services()",
    ])
    return lines
