...
def export_workflow_python(data: Mapping[str, Any], *, style: str = "flat") -> str:
    ...
    lines = [
        ...
        "    ):",
        "        try:",
        "            status = importlib.import_module(module_name).runtime_status()",
        "        except Exception:",
        "            continue",
        "        for key in ('cv2_streams', 'streams', 'reasoning_streams'):",
        "            for item in status.get(key, []) or []:",
        "                if not isinstance(item, dict):",
        "                    continue",
        "                name = item.get('stream_id') or item.get('topic') or item.get('device') or '?'",
