    ...
    >
      ...
      {expanded && (
        <div className="bn-run-detail">
          ...
          <div className="bn-run-detail-actions">
            {isRunning
              ? <button onClick={onStop} disabled={busy} style={miniButton}>{isMapping ? 'Stop mapping' : 'Stop'}</button>
              : <button onClick={onStart} disabled={busy} style={primaryButton}>{isMapping ? 'Start mapping' : 'Run'}</button>}
            {isRunning && isMapping && (
              <button onClick={onSaveMap} disabled={busy} style={primaryButton}>
                Save map
              </button>
            )}
          ...
          {isRunning && isMapping && (
            <LiveOccupancyMap
              ...
              topic={deployment.mapping_topic || '/map'}
            />
          )}
        </div>
      )}
